...
@servico.get("/likes_por_historia/<int:historia_id>")
def get_likes_por_historia(historia_id):
    conexao = get_conexao_com_bd()
    cursor = conexao.cursor(dictionary=True)
    cursor.execute("SELECT count(*) as quantidade " +  
        "FROM likes " +
        "WHERE likes.feed = " + str(historia_id)
    )
...
def curtir(conta, id_do_feed):
    ...
    try:
        ...
        conexao.rollback()
        resultado = jsonify(situacao = "erro", erro = "erro curtindo a historia")
...
def descurtir(conta, id_do_feed):
    ...
    try:
        ...
        conexao.rollback()
        resultado = jsonify(situacao = "erro", erro = "erro descurtindo a historia")
